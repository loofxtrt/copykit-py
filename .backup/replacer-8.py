...
def resolve_context_paths(data: dict):
    ...
    raw_target_parent = context.get('target-parent')
    raw_substitute_parent = context.get('substitute-parent')
    id = context.get('id')
    ...
    substitute_parent = Path(raw_substitute_parent.replace('ROOT', str(SUBSTITUTES)))
...
def load_mapping() -> Mapping:
    for f in INSTRUCTIONS.iterdir():
        ...
        context = data.get('context')
        id = context.get('id')
        target_parent = context.get('target-parent')
