...
def handle_create_or_replace(
    ...
    if not substitute_path.exists() or not substitute_path.is_file():
        logger.error(f'substituto inválido: {substitute_path}')
        continue
    ...
    if action == 'replace':
        ...
        copy(substitute=substitute_path, destination=destination, operation='substituído')
    elif action == 'create':
        copy(substitute=substitute_path, destination=destination, operation='criado')
