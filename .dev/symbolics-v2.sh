...
whole_sections() {
    clean_and_copy "$FLUENT/16/actions" "$COPYCAT/actions/16"
    clean_and_copy "$FLUENT/symbolic/actions" "$COPYCAT/actions/symbolic"

    # FIXME: os apps tem o símbolo de help e o de system que o menu usa
    # então eles sobreescrever os de 22
    clean_and_copy "$FLUENT/symbolic/apps" "$COPYCAT/apps/symbolic"

    clean_and_copy "$FLUENT/22/categories" "$COPYCAT/categories/symbolic"

    clean_and_copy "$FLUENT/22/devices" "$COPYCAT/devices/symbolic"

    clean_and_copy "$FLUENT/16/devices" "$COPYCAT/devices/16"
    clean_and_copy "$FLUENT/22/devices" "$COPYCAT/devices/22"
    clean_and_copy "$FLUENT/24/devices" "$COPYCAT/devices/24"

    clean_and_copy "$FLUENT/symbolic/status" "$COPYCAT/status/symbolic"
}
...
#     clean_and_copy "$FLUENT/symbolic/devices" "$COPYCAT/devices/symbolic"

#     clean_and_copy "$FLUENT/symbolic/status" "$COPYCAT/status/symbolic"

#     clean_and_copy "$FLUENT/symbolic/mimetypes" "$COPYCAT/mimetypes/symbolic"

#     clean_and_copy "/mnt/seagate/symlinks/copydb/original-unzipped/Fluent/16/panel" "$COPYCAT/panel/16"
#     clean_and_copy "/mnt/seagate/symlinks/copydb/original-unzipped/Fluent/22/panel" "$COPYCAT/panel/22"
#     clean_and_copy "/mnt/seagate/symlinks/copydb/original-unzipped/Fluent/24/panel" "$COPYCAT/panel/24"
